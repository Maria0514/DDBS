...
def initialize_demo_data():
    ...
    # 确保账户存在并有足够的余额
    banking_service.delete_account(3001)
    banking_service.delete_account(3002)
    banking_service.create_account(3001, 1000.0)
    banking_service.create_account(3002, 1500.0)

    # 确保产品存在并有足够的库存
    conn = get_db_manager().get_connection('db1')
    cursor = conn.cursor()
    cursor.execute("DELETE FROM inventory WHERE product_id = %s", (301,))
    cursor.execute("""
        INSERT IGNORE INTO inventory (product_id, product_name, quantity, price)
        VALUES (301, 'Laptop', 10, 999.99)
    """)
    conn.commit()
    cursor.close()
    conn.close()
...
def run_concurrent_transactions():
    ...
    thread1 = threading.Thread(target=transfer_task, args=(3001, 3002, 500))
    
    # 转账任务2：从账户2001转600到2002（失败，余额不足）
    thread2 = threading.Thread(target=transfer_task, args=(3001, 3002, 600))
    
    # 库存任务1：订购产品101，数量5（成功）
    thread3 = threading.Thread(target=inventory_task, args=(301, 5, 3001))
    
    # 库存任务2：订购产品101，数量10（失败，库存不足）
    thread4 = threading.Thread(target=inventory_task, args=(301, 10, 3002))
